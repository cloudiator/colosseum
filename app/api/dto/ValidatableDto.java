...
import api.validation.ValidateBuilder;
import api.validation.ValidationException;
import api.validation.ValidationHolder;
 ...
 */
public abstract class ValidatableDto extends GetDto implements Validatable {
    ...
    @Nullable @Override public final List<ValidationError> validate() {
        ...
        try {
            errors.addAll(validationHolder.validate().stream()
                .map(api.validation.ValidationError::toPlayError).collect(Collectors.toList()));
