...
public class ImportHardwareToDatabase implements Solution {
    ...
    @Inject
    public ImportHardwareToDatabase(HardwareModelService hardwareModelService,
    ...
    public void applyTo(Problem problem) throws SolutionException {
        ...
        Hardware existingHardware = hardwareModelService.getByRemoteId(
                SlashEncodedId.of(hardwareNotInDatabase.getResource().id()).cloudId());
        if (existingHardware != null) {
            existingHardware
                    .addCloudCredential(hardwareNotInDatabase.getResource().credential());
            return;
        }

        Cloud cloud = hardwareNotInDatabase.getResource().cloud();
        Location location = null;
        if (hardwareNotInDatabase.getResource().location().isPresent()) {
            location = locationModelService.getByRemoteId(SlashEncodedId
                    .of(hardwareNotInDatabase.getResource().location().get().id()).cloudId());
            if (location == null) {
                throw new SolutionException(String.format("Could not import hardware %s as location %s is not (yet) imported.", hardwareNotInDatabase.getResource(), hardwareNotInDatabase.getResource().location().get()));
            }
        }

        //todo check this
        Hardware hardware = new Hardware(hardwareNotInDatabase.getResource().cloudId(),
                hardwareNotInDatabase.getResource().providerId(),
                hardwareNotInDatabase.getResource().swordId(), cloud, location,
                hardwareNotInDatabase.getResource().name(),
                getHardwareOffer(hardwareNotInDatabase.getResource().numberOfCores(),
                        hardwareNotInDatabase.getResource().mbRam(),
                        hardwareNotInDatabase.getResource().gbDisk()));
