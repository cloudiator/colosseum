...
public class WindowsInstaller extends AbstractInstaller {
    ...
    private static final String SEVEN_ZIP_ARCHIVE = "7zip.zip";
    ...
    private static final String SEVEN_ZIP_DOWNLOAD = Play.application().configuration().getString("colosseum.installer.windows.java.download");
    ...
    private void install7Zip(){
        ...
        this.remoteConnection.executeCommand("powershell -command & { Add-Type -A 'System.IO.Compression.FileSystem'; [IO.Compression.ZipFile]::ExtractToDirectory('" + this.homeDir + "7zip.zip', '" + this.homeDir + "\\"+ this.SEVEN_ZIP_DIR + "'); }");
    ...
    public void installKairosDb() {
        ...
        //extract kairosdb
        this.remoteConnection.executeCommand("powershell -command " + this.homeDir +"\\"+ WindowsInstaller.SEVEN_ZIP_DIR +"\\7za.exe e " + this.homeDir + "\\"+ WindowsInstaller.KAIROSDB_ARCHIVE + " -o" + this.homeDir);
        String kairosDbTar = WindowsInstaller.KAIROSDB_ARCHIVE.replace(".gz", "");
        this.remoteConnection.executeCommand("powershell -command " + this.homeDir + "\\" + WindowsInstaller.SEVEN_ZIP_DIR + "\7za.exe x " + this.homeDir + "\\" + kairosDbTar + " -o" + this.homeDir);
