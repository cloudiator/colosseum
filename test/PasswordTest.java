...
public class PasswordTest {
	...
	public void testGenerateSalt() {
		...
		assertThat(Password.getInstance().generateSalt(),not(equalTo(Password.getInstance().generateSalt())));
	}
	 ...
	 */
	@Test(expected=IllegalArgumentException.class)
	public void testHashEmptyPasswordAndSalt() {
	...
	public void testHash() {
		...
		assertArrayEquals(Password.getInstance().hash(password, salt), Password.getInstance().hash(password, salt));
		
		//the hash of the same password but different salt must not be equal
		assertThat(Password.getInstance().hash(password, salt), not(equalTo(Password.getInstance().hash(password, saltIncorrect))));
		
		//the hash of the same salt but different password must not be equal
		assertThat(Password.getInstance().hash(password, salt), not(equalTo(Password.getInstance().hash(passwordIncorrect, salt))));
